...
(function () {
  ...
  var PERSONAS = {
    ...
    // Eintretende Elemente laufen "out", nie "inOut"/"in": ease-in verzoegert genau den
    // Moment, den der Nutzer anschaut, und fuehlt sich traege an (review-animations, Standard 3).
    // Der mechanische Charakter kommt hier aus kurzer Dauer + kurzem Weg, nicht aus der Kurve.
    mechanical: {
      lenis: { duration: 0.7, easing: EASE.quadOut },
      ease: 'power2.out', dur: { base: 0.5, mask: 0.5, wipe: 0.5 },
      dist: { y: 24, x: 24, scale: 0.96 }, start: 'top 90%', staggerStep: 0.04,
      vocab: ['up', 'left', 'right'],
      hero: { y: 18, dur: 0.5, ease: 'power2.out', stagger: 0.05, delay: 0.1 },
  ...
  if (!off && persona.lenis && typeof window.Lenis !== 'undefined') {
    ...
    document.querySelectorAll('a[href^="#"]').forEach(function (a) {
      a.addEventListener('click', function (e) {
        ...
        if (id.length > 1 && document.querySelector(id)) { var off = parseFloat(a.getAttribute('data-offset')); e.preventDefault(); lenis.scrollTo(id, { offset: isNaN(off) ? -70 : -off }); }
      });
    });
  }
