...
const seiten = process.argv[2] ? process.argv[2].split(',') : ['index', 'kelleraussenabdichtung', 'kontakt', 'impressum'];
const breiten = process.argv[3] ? process.argv[3].split(',').map(Number) : [1440, 834, 390];
const nurFold = process.argv.includes('--fold');
for (const s of seiten) {
  for (const w of breiten) {
    ...
    await page.goto('http://localhost:8793/' + s + '.html', { waitUntil: 'networkidle' });
    if (!nurFold) {
    ...
    await page.screenshot({ path: join(outDir, s + '-' + tag + (nurFold ? '-fold' : '') + '.png'), fullPage: !nurFold });
