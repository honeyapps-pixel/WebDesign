...
def karte_html():
  ...
  <div class="karte" data-karte data-marker="{data}">
    <div class="karte__consent">
      {GIEBEL}
      <p class="karte__consent-t">Referenz-Karte: Wo wir gebaut haben.</p>
      <p class="karte__consent-s">Die Karte lädt Kartenkacheln von OpenStreetMap. Dabei wird Ihre IP-Adresse an die OpenStreetMap Foundation übertragen. <a href="datenschutz.html#karte">Mehr im Datenschutz</a></p>
      <button type="button" class="btn btn--solid" data-karte-laden>Karte laden {I_PFEIL}</button>
      <a class="karte__consent-alt" href="https://www.openstreetmap.org/?mlat={SITZ['lat']}&amp;mlon={SITZ['lon']}#map=11/{SITZ['lat']}/{SITZ['lon']}" target="_blank" rel="noopener">oder auf openstreetmap.org öffnen</a>
    </div>
  </div>
...
def seite_datenschutz():
  ...
  <h3>Vercel</h3>
  <p>Anbieter ist die Vercel Inc., 440 N Barranca Ave #4133, Covina, CA 91723, USA (nachfolgend Vercel). Wenn Sie unsere Website besuchen, werden Ihre personenbezogenen Daten (z. B. IP-Adresse, Zeitpunkt des Zugriffs, aufgerufene Seite, Browsertyp) auf den Servern von Vercel verarbeitet; die Auslieferung erfolgt über Vercels weltweites Netzwerk mit Standorten auch in der EU (z. B. Frankfurt). Details entnehmen Sie der Datenschutzerklärung von Vercel: <a href="https://vercel.com/legal/privacy-policy" target="_blank" rel="noopener">https://vercel.com/legal/privacy-policy</a>.</p>
  <p>Die Verwendung von Vercel erfolgt auf Grundlage von Art. 6 Abs. 1 lit. f DSGVO. Wir haben ein berechtigtes Interesse an einer möglichst zuverlässigen Darstellung unserer Website. Eine Übermittlung in die USA stützt sich auf die Standardvertragsklauseln der EU-Kommission (Art. 46 Abs. 2 lit. c DSGVO) im Auftragsverarbeitungsvertrag mit Vercel.</p>
  ...
  <p>Diese Website setzt keine Cookies und speichert keine Informationen in Ihrem Endgerät (kein Local Storage, kein Tracking, keine Analyse- oder Werbedienste). Deshalb wird auch keine Einwilligung nach § 25 TTDSG abgefragt. Technisch notwendige Verbindungsdaten fallen ausschließlich beim Hosting (Abschnitt 2) an.</p>
  <h2 id="karte">6. Kartendarstellung (OpenStreetMap)</h2>
  <p>Auf der Kontaktseite bieten wir eine Karte des Dienstes OpenStreetMap an, bereitgestellt von der OpenStreetMap Foundation, St John's Innovation Centre, Cowley Road, Cambridge, CB4 0WS, Großbritannien. Die Karte wird <b>erst geladen, wenn Sie auf „Karte laden" klicken</b>. Erst dann werden Kartenkacheln vom Server der OpenStreetMap Foundation abgerufen und dabei Ihre IP-Adresse übertragen. Rechtsgrundlage ist Ihre Einwilligung durch den Klick (Art. 6 Abs. 1 lit. a DSGVO); sie gilt für den jeweiligen Seitenaufruf und lässt sich durch Neuladen der Seite widerrufen. Die Kartenbibliothek (Leaflet) liegt auf unserem eigenen Server. Weitere Informationen: <a href="https://wiki.osmfoundation.org/wiki/Privacy_Policy" target="_blank" rel="noopener">wiki.osmfoundation.org/wiki/Privacy_Policy</a>.</p>
  <h2>7. Schriften, Skripte und externe Links</h2>
  <p>Schriften und Skripte dieser Website werden lokal von unserem Server geladen; es findet keine Verbindung zu Google Fonts oder anderen Schriftdiensten statt. Links zu externen Diensten (WhatsApp, Google Maps für Route und Rezensionen, Facebook) öffnen den jeweiligen Dienst erst, wenn Sie den Link anklicken; bis dahin werden keine Daten an diese Anbieter übertragen.</p>
  <p class="hinweis">Stand: September 2026. Diese Datenschutzerklärung basiert auf dem Entwurf des Betriebs und wurde an die tatsächlich auf dieser Website eingesetzten Dienste angepasst.</p>
</div></section>'''
    return seite(datei="datenschutz.html", titel="Datenschutzerklärung | BAUSCHULZ GmbH & Co. KG", beschreibung="Datenschutzerklärung der BAUSCHULZ GmbH & Co. KG, Fehringstraße 8, 38524 Sassenburg – Hosting, Kontakt, Cookies, Karte.", aktiv="", body=body)
